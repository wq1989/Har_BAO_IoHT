classdef confusion_2
    methods (Static)
        function [c_matrix,Result,RefereceResult]= getMatrix(actual,predict,Display)
            ...
            %%http://www.dataschool.io/simple-guide-to-confusion_2-matrix-terminology/
            %%https://en.wikipedia.org/wiki/confusion_2_matrix
            ...
            % [c_matrix,Result,RefereceResult]= confusion_2mat(actual,predict)
            %
            % %DIsplay off
            % % [c_matrix,Result,RefereceResult]= confusion_2mat(actual,predict,0)
            %
            % %%
            % %single Class
            % n=100;m=1;
            % actual=round(rand(1,n)*m);
            % predict=round(rand(1,n)*m);
            % [c_matrix,Result]= confusion_2mat(actual,predict)
            ...
            %disp('confusion_2 Matrix')
            %disp(c_matrix_table)
            [Result,RefereceResult]=confusion_2.getValues(c_matrix);
            ...
            if Display
                if n_class>2
                    disp('Multi-Class confusion_2 Matrix Output')
        ...
        function  [Result,RefereceResult]= getValues(c_matrix)
            ...
            if row~=col
                error('confusion_2 matrix dimention is wrong')
            end
            n_class=row;
            switch n_class
                case 2
